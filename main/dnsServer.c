...
#define STEP_LOGD(...) ESP_LOGI(__VA_ARGS__)
#else
#define STEP_LOGI(...) while (0)
#define STEP_LOGW(...) while (0)
#define STEP_LOGE(...) ESP_LOGE(__VA_ARGS__)
#define STEP_LOGD(...) while(0)
#endif
...
static const char *DNSTAG = "example_dns_redirect_server";
...
static int parse_dns_request(char *req, size_t req_len, char *dns_reply, size_t dns_reply_max_len)
{
    ...
    STEP_LOGD(DNSTAG, "DNS query with header id: 0x%X, flags: 0x%X, qd_count: %d",
    ...
    for (int i = 0; i < qd_count; i++) {
        ...
        if (name_end_ptr == NULL) {
            STEP_LOGE(DNSTAG, "Failed to parse DNS question: %s", cur_qd_ptr);
            return -1;
        }
        ...
        STEP_LOGD(DNSTAG, "Received type: %d | Class: %d | Question for: %s", qd_type, qd_class, name);
        ...
        if (qd_type == QD_TYPE_A) {
            ...
            STEP_LOGD(DNSTAG, "Answer with PTR offset: 0x%X and IP 0x%X", ntohs(answer->ptr_offset), ip_info.ip.addr);
...
void dns_server_task(void *pvParameters)
{
    ...
    while (1) {
        ...
        if (sock < 0) {
            STEP_LOGE(DNSTAG, "Unable to create socket: errno %d", errno);
            break;
        }
        STEP_LOGI(DNSTAG, "Socket created");

        int err = bind(sock, (struct sockaddr *)&dest_addr, sizeof(dest_addr));
        if (err < 0) {
            STEP_LOGE(DNSTAG, "Socket unable to bind: errno %d", errno);
        }
        STEP_LOGI(DNSTAG, "Socket bound, port %d", DNS_PORT);

        while (1) {
            STEP_LOGI(DNSTAG, "Waiting for data");
            ...
            if (len < 0) {
                STEP_LOGE(DNSTAG, "recvfrom failed: errno %d", errno);
                close(sock);
                break;
            }
            // Data received
                ...
                STEP_LOGI(DNSTAG, "Received %d bytes from %s | DNS reply with len: %d", len, addr_str, reply_len);
                if (reply_len <= 0) {
                    STEP_LOGE(DNSTAG, "Failed to prepare a DNS reply");
                } else {
                    int err = sendto(sock, reply, reply_len, 0, (struct sockaddr *)&source_addr, sizeof(source_addr));
                    if (err < 0) {
                        STEP_LOGE(DNSTAG, "Error occurred during sending: errno %d", errno);
                        break;
                    }
                }
            }
        ...
        if (sock != -1) {
            STEP_LOGE(DNSTAG, "Shutting down socket");
            shutdown(sock, 0);
            close(sock);
        }
    }
